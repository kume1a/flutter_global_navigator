...
typedef OnTap = void Function(GNSnackBar snack);

typedef SnackbarStatusCallback = void Function(SnackbarStatus? status);

class GNSnackBar extends StatefulWidget {
  const GNSnackBar({
    Key? key,
    this.title,
    this.message,
  ...
  @override
  State createState() => GNSnackBarState();

  /// Show the snack. It's call [SnackbarStatus.opening] state
  /// followed by [SnackbarStatus.open]
  SnackbarController show() {
    return GlobalNavigator.showSnackbar(this);
  }
}

class GNSnackBarState extends State<GNSnackBar> with TickerProviderStateMixin {
...
You need to either use message[String], or messageText[Widget] or define a userInputForm[Form] in GetSnackbar''',
  ...
  void _configurePulseAnimation() {
    ...
    _fadeAnimation = Tween<double>(begin: _initialOpacity, end: _finalOpacity).animate(
      CurvedAnimation(
  ...
  Widget _containerWithoutForm() {
    final double iconPadding = widget.padding.left > 16.0 ? widget.padding.left : 0.0;
    final double left =
